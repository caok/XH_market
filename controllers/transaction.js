...
var AlipayConfig = require('../alipay_config');
...
/**
 * TODO alipayto与payment结合
 * 在应用中发送付款请求，替换掉构造form的应用
 */
function alipayto(order, res) {
...
//                                   /gateway.do?body=11&notify_url=http://www.xxx.cn/create_direct_pay_by_user_jsp_utf8/notify_url.jsp&out_trade_no=20120708132324&payment_type=1&return_url=http://127.0.0.1:8080/create_direct_pay_by_user_jsp_utf8/return_url.jsp&service=create_direct_pay_by_user&show_url=http://www.xxx.com/order/myorder.jsp&subject=11&total_fee=0.01&sign=dfc1995af2ff01642a3cf6936ce0d57c&sign_type=MD5
    ...
    //请与贵网站订单系统中的唯一订单号匹配
    var out_trade_no = order._id;
    //订单名称，显示在支付宝收银台里的“商品名称”里，显示在支付宝的交易管理的“商品名称”的列表里。
    if (order.products[0]){
      var subject = order.products[0].name['zh'];
    }else{
      var subject = order._id;
    };
    //订单描述、订单详细、订单备注，显示在支付宝收银台里的“商品描述”里
    var body = "";
    //订单总金额，显示在支付宝收银台里的“应付总额”里
    var total_fee = order.pay;
    ...
    var show_url = "http://" + config.host + "/myorder";
...
//    console.log(req.query());
    ...
    AlipayNotify.verity(params,function(result){
      if(result){
        Order.findOne({_id: order_no}, function(err, order){
          if (order.state == 0){
      ...
      if (product){
        ...
        if (idx == pds.length - 1){
          ...
          order.save(function(err){
            ...
            setTimeout(function(){
              Order.findOne({_id: order._id}, function(err, o){
                if (o.state == 0){
                  o.paying = 0;
                  o.save(function(err){
                    searchAndUpdatePds(o.products.length-1, o.products, o, false, false);
                  });
                }
              });
            }, 1000*60*10);
